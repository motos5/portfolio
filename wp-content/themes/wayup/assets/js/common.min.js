jQuery(document).ready(function($){
	...
	// Получаем объекты для перевода текста в тегах <li>
	const menu_order = wnm_menu_order['wayup-js-main'];
	const popularity = wnm_popularity['wayup-js-main'];
	const rating = wnm_rating['wayup-js-main'];
	const date = wnm_date['wayup-js-main'];
	const price = wnm_price['wayup-js-main'];
	const price_desc = wnm_price_desc['wayup-js-main'];


	// Получаем изменённую конечную часть ссылки
	const queryString = window.location.search;
	const orderLink = queryString.slice(9);

	// Пишем условие для изменения текста в тегах <li>
	if($('.sort-menu li').length) {
		let order_text = 'Default sorting';

		if(orderLink == 'popularity') {
			order_text = popularity;
		} else if(orderLink == 'rating') {
			order_text = rating;
		} else if(orderLink == 'date') {
			order_text = date;
		} else if(orderLink == 'price') {
			order_text = price;
		} else if(orderLink == 'price-desc') {
			order_text = price_desc;
		} else {
			order_text = menu_order;
		}

		$('#parametr').text(order_text);
	};

	// Получаем из объекта ссылку на страницу
	const shoppage_url = wnm_shoppage_url['wayup-js-main'];

	$('.sort-menu li').on('click',function() {
		// Для WooCommerce сортировки
		window.location = shoppage_url + '?orderby=' + $(this).find('a').data('value');
	});
	...
	/*-------------------Hide long text----------------------*/
	// Buttons Objects from - functions.php
	const collapse = wnm_collapse['wayup-js-main'];
	const readMore = wnm_read_more['wayup-js-main'];

	$('.is-home .clients__text').dotdotdot();

	$('.inner.clients .clients__text').each(function(){
		var $dot5 = $(this);
			$dot5.append( ' <a class="clients__more link-more" href="#"><span class="open">' + readMore + '</span><span class="close">' + collapse + '</span></a>' );
